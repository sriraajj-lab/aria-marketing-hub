...
export function DenialDetailView() {
  ...
  const [pipelineSteps, setPipelineSteps] = useState<Array<{ agent: string; task: string; success: boolean; validated: boolean; confidence: number; validationErrors?: string[] }>>([]);
  ...
  const runAnalysis = async () => {
    ...
    try {
      // Route through Conductor instead of direct API call
      const res = await fetch('/api/conductor', {
        ...
        body: JSON.stringify({
          ...
          level: accessLevel,
          input: { denialId: selectedDenialId },
        }),
      });
      const result = await res.json();
      // Refresh denial data from the data layer
      await fetchDenial();
      if (result.success) {
        toast.success('Denial analysis completed successfully');
      } else {
        const errors = result.steps?.[0]?.validationErrors?.join(', ');
        toast.error(errors || 'Failed to analyze denial');
      }
    } catch (error) {
  ...
  const runCorrection = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errors = result.steps?.[0]?.validationErrors?.join(', ');
        toast.error(errors || 'Failed to generate correction suggestion');
      }
    } catch (error) {
  ...
  const runQualityCheck = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errors = result.steps?.[0]?.validationErrors?.join(', ');
        toast.error(errors || 'Failed to run quality check');
      }
    } catch (error) {
  ...
  const runFullPipeline = async () => {
    if (!selectedDenialId || !accessLevel) return;
    ...
    try {
      ...
      await fetchDenial();
      if (result.success) {
        toast.success(`Pipeline completed: ${result.steps?.filter((s: any) => s.success).length}/${result.steps?.length} steps succeeded`);
      } else {
        const failedStep = result.steps?.find((s: any) => !s.success);
        toast.error(`Pipeline stopped at ${failedStep?.agent || 'unknown'}: ${failedStep?.validationErrors?.join(', ') || 'failed'}`);
      }
    } catch (error) {
          ...
          <Card className="border-border bg-card">
            ...
            <CardContent className="p-4 space-y-3">
              {isL1 ? (
                ...
                /* Level 2+: Full agent actions via Conductor */
                <>
                  {/* Full Pipeline Button */}
                  <Button
                    onClick={runFullPipeline}
                    disabled={runningPipeline || denial.status === 'Reviewed' || denial.status === 'Resubmitted' || denial.status === 'Closed'}
                    className="w-full bg-gradient-to-r from-primary to-emerald hover:opacity-90 text-white"
                  >
                    {runningPipeline ? (
                      <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Running Pipeline...</>
                    ) : (
                      <><Zap className="h-4 w-4 mr-2" /> Run Full Pipeline</>
                    )}
                  </Button>
                  <Separator className="bg-border" />
                  ...
                  {pipelineSteps.length > 0 && (
                    <div className="rounded-lg bg-secondary p-3 space-y-2">
                      <p className="text-xs font-medium text-foreground">Pipeline Steps</p>
                      {pipelineSteps.map((step, idx) => (
                        <div key={idx} className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">{step.agent.replace(/-/g, ' ')}</span>
                          <span className="text-muted-foreground">·</span>
                          <span className={step.validated ? 'text-emerald' : 'text-yellow-400'}>
                            {step.validated ? 'validated' : 'unvalidated'}
                          </span>
                          <span className="text-muted-foreground">·</span>
                          <span className="text-foreground">{(step.confidence * 100).toFixed(0)}%</span>
                          {step.validationErrors && step.validationErrors.length > 0 && (
                            <Badge variant="outline" className="text-[8px] bg-red-500/10 text-red-400 border-red-500/30">
                              {step.validationErrors.length} errors
                            </Badge>
                          )}
                        </div>
                      ))}
