...
export const NCCI_EDIT_PAIRS: Array<[string, string, boolean]> = [
...
export function getCARCSpecificGuidance(carcCode: string): {
  ...
  const guidance: Record<string, { commonFixes: string[]; successRate: number; typicalResolutionDays: number }> = {
    ...
    'CO-97': {
      commonFixes: [
        'Verify ASP (Average Sales Price) rate per CMS quarterly update for J-codes',
        ...
        'Compare billed rate against Medicare fee schedule or contracted rate',
      ],
      successRate: 45,
      typicalResolutionDays: 35,
    },
    'CO-5': {
      commonFixes: [
        'Verify CPT code matches the procedure performed and documented',
        'Check if CPT code is valid for the place of service billed',
        'Ensure CPT code is not deleted/revised for the date of service',
        'Verify CPT-to-revenue code mapping for facility billing',
      ],
      successRate: 82,
      typicalResolutionDays: 21,
    },
    'CO-9': {
      commonFixes: [
        'Verify diagnosis code supports the medical necessity of the procedure',
        'Check CPT-ICD crosswalk for covered diagnoses',
        'Add secondary diagnoses that support the procedure',
        'Ensure diagnosis is coded to the highest level of specificity',
      ],
      successRate: 70,
      typicalResolutionDays: 25,
    },
    'CO-96': {
      commonFixes: [
        'Verify the service is a covered benefit under the patient\'s plan',
        'Check if prior authorization was required but not obtained',
        'Determine if service exceeds frequency limitations',
        'Review LCD/NCD for coverage criteria that may apply',
      ],
      successRate: 40,
      typicalResolutionDays: 40,
    },
  };
