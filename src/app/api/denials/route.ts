...
import { detectSpecialty, getSpecialtyDefinition, type SpecialtyName } from '@/lib/specialties';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (specialty) {
      filtered = filtered.filter((d) => (d as Record<string, unknown>).specialty === specialty);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Auto-detect specialty from CPT + ICD-10 if not provided
    if (!body.specialty && body.cptCode && body.diagnosisCode) {
      const detected = detectSpecialty(body.cptCode, body.diagnosisCode);
      const specDef = getSpecialtyDefinition(detected);
      body.specialty = detected;
      body._specialtyInfo = {
        detected,
        displayName: specDef.displayName,
        riskLevel: specDef.denialRiskLevel,
        hasJCodes: specDef.hasJCodes,
        hasTimeBasedCoding: specDef.hasTimeBasedCoding,
      };
    } else if (!body.specialty) {
