...
// Legacy batch job storage (for compatibility with old API)
const legacyJobs: Map<string, Record<string, unknown>> = new Map();
function createBatchJob(data: Record<string, unknown>) {
  const job = { id: `legacy-${Date.now()}`, ...data, status: 'pending', createdAt: new Date().toISOString() };
  legacyJobs.set(job.id, job);
  return job;
}
function getBatchJob(id: string) { return legacyJobs.get(id); }
function getAllBatchJobs(_filter?: Record<string, unknown>) { return Array.from(legacyJobs.values()); }
function updateBatchJob(id: string, data: Record<string, unknown>) {
  const job = legacyJobs.get(id);
  if (job) Object.assign(job, data);
  return job;
}
function addBatchResult(id: string, result: Record<string, unknown>) {
  const job = legacyJobs.get(id);
  if (job && Array.isArray(job['results'])) { (job['results'] as unknown[]).push(result); }
  else if (job) { job['results'] = [result]; }
}
